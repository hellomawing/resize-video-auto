...
def build_spec(settings: dict, extra_archive_dirs=None) -> dict:
    ...
    return {
        ...
        # 指定的 + 历史默认名 + **曾经用过的名字**。用集合而不是单个名字 ——
        # 归档目录能按监控目录分别设置，只排除一个名字的话，别处归档走的原片
        # 会被当成新视频再切一遍。
        #
        # 「曾经用过的名字」由 config.collect_archive_dirs 顺手落盘（只增不减）：
        # 只按当前配置现算的话，名字一改，旧归档目录里的原片（保留着原文件名）
        # 就会掉出排除集被重切。见 README「原片处理方式」一节。
        #
