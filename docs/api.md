...
- `parent` 是上一级目录，**只在它仍落在可访问范围内**时才下发；浏览挂载根本身时是
  `null`（前端据此禁用「上一级」按钮）—— 挂进来的往往是某个深层子目录，它的父目录
  （`/`、`/vol1/1000`）在容器里根本不可见，给出来只会点出一个 403。
...
>
> 选择器**不提供手动输入路径**，界面上自上而下三步：**① 选存储位置**（即 `roots`，
> 容器挂载进来的目录）→ **② 选用使用方式**（直接用这个位置 / 选它的子文件夹）→
> **③ 仅在选「子文件夹」时**展开逐级浏览（`dirs` + `parent` + 上述两条捷径）。
> 免得用户有机会敲进一条走不通的路径。要添加全新目录，把它挂载进容器（docker-compose 的
> volumes）即可。
...
#### 命中预览 `POST /api/watchpoints/filter-preview`

编辑页在规则下面放了个「命中预览」输入框，敲一条样例路径就能立刻看到
它会不会被挡下。**判定与真实扫描同源**（同一个 `filters.explain`），
所以这里说会处理、扫描时就真的会处理 —— 预览说会处理而实际跳过，
比压根没有预览更糟。

```json
// 请求
{
  "path": "相机导入/2026/DJI_0002.mp4",
  "basePath": "/vol1/media/inbox",
  "filters": {
    "extInclude": [".mp4"],
...
- **纯计算**：不读磁盘、不写配置、不碰扫描队列，可以随便点。`path`
  不要求真实存在，也不要求落在可浏览范围内 —— 用户往往是拿一个脑子里的
  文件名去试规则，而不是先去目录里翻出一个真实文件。
- `basePath` 传当前监控目录，这样把**整条绝对路径**粘进来也能相对化；
  不给就按相对路径处理。
- `parts` 照实回显**实际参与比对的每一段名字**（含文件名自己）。规则到底
  在跟什么比，看这一列最清楚 —— 尤其是用正则写「含 A 且不含 B」的时候，
  正则跨不了路径段这个坑看一眼就明白了。
- `hasRules: false` = 一条规则都没配，什么都不会被挡。
- `skipped` 为真时 `reason` 与 `ScanResult.ignored[].reason` **文案同源**。
- 规则本身写坏（正则编译不过）时**仍然是 200**，用 `ok: false` + `message`
  说明是哪一条：预览是「帮我看看」而非「保存配置」，用 400 会逼前端去区分
  「网络坏了」和「正则写错了」。
