...
- 白名单 = 设置 `watch.allowedRoots` **∪ 容器内挂载的数据目录**（自动探测
  `/proc/self/mountinfo`，排除伪文件系统、根 overlay 与 `/data` 卷）——
  在 Docker 里显式挂载过的目录自动可选，无需再到设置里手填。
- 传入路径不在白名单内时返回 `403`。
...
- `suggestedRoots`：某个根**不可枚举**时（fnOS 把 /vol1 权限位设成 000，readdir
  ...
  范围 0~2999，最多 8 个）。前端摆成「点一下直达」入口 —— 用户不必改白名单
...
> 敲进一条走不通的路径。要添加全新目录，把它挂载进容器即可（自动并入白名单），
> 或把 `allowedRoots` 改到可枚举的层。
...
- `watch.allowedRoots`：可访问根目录白名单，约束目录浏览与监控目录添加；
  容器内挂载的数据目录会自动并入这份白名单（见 `/api/browse` 说明），无需重复填写
