...
```
扫描完成：没有需要处理的新视频。目录里有 1 个视频文件被跳过
（1 个是本工具切出来的切片或已标记的原片）——跳过是为了防止把自己切出来的
半成品再切一遍。想把原片重新切一次，去「撤销分割」页恢复它的原名。
```

如果被跳过的是「切片已经不在了的原片」（切片被删掉或搬走了），它会出现在
**「撤销分割」页**一个专门的区块里，点「恢复原名」就能让它重新变成待处理的
普通文件。这类文件以前在界面上是完全隐身的——扫描跳过它，撤销页又只从切片
出发去找原片，两头都看不见，只能靠命令行改名。
